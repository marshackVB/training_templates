...
class SkLearnHyperoptBase(SkLearnPipelineABC):
    ...
    Note: 
        ...
        The instance attribute, self.model_name, should set by the inheriting class. This is a string represention of the
        model type and is used to name MLflow experiment runs.
    ...
    def __init__(self, delta_feature_table:str, delta_train_val_id_table:str, train_size:float, numerical_cols:List[str], categorical_cols:List[str], 
                 binary_cols:List[str], label_col:str, problem_type:str, hyperparameter_space:Dict[str, Any], hyperopt_max_evals:int, hyperopt_iteration_stop_count:int,
                 hyperopt_early_stopping_threshold:float, mlflow_experiment_location:str, mlflow_run_description:str, random_state:int=123, train_eval_shuffle:bool=True, 
                 commit_hash:str=None, release_version:str=None):
        ...
        self.logging_attributes_to_exclude = ['feature_df', 'model', 'X_train', 'X_val', 'y_train', 'y_val',
    ...
    def _join_train_val_features(self):
        ...
        train_val_features = features.join(train_val_ids, [train_val_ids_primary_key], 'inner')
    ...
    def init_preprocessing_pipeline(self) -> ColumnTransformer:
        ...
        preprocessing_pipeline = ColumnTransformer([('categorical_cols', categorical_transform, self.categorical_cols),
    ...
    def init_model_pipeline(self, model_params: Dict[str, Any]) -> Pipeline:
        ...
        classification_pipeline = Pipeline([("preprocessing_pipeline", preprocessing_pipeline), ("model", model)])
    ...
    def train_test_split(self) -> None:
        ...
        """
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(self.feature_df[self.all_feature_cols],
    ...
    def config_hyperopt_objective_fn(self, X_train_transformed, X_val_transformed) -> Callable:
        """
        Return a Hyperopt object function for the type of model to be trained
        """
        raise NotImplementedError("You must override this method with a Hyperopt ojective function for your model")
    ...
    def tune_hyperparameters(self) -> Dict[str, Union[str,int,float]]:
        ...
        object_fn = self.config_hyperopt_objective_fn(X_train_transformed, X_val_transformed)

        trials = Trials()

        print("Beginning hyperopt parameter search")

        early_stop_fn = no_progress_loss(iteration_stop_count=self.hyperopt_iteration_stop_count, 
                                         percent_increase=self.hyperopt_early_stopping_threshold)

        best_params = fmin(fn=object_fn, 
    ...
    def train(self) -> None:
        ...
        with mlflow.start_run(run_name=self.model_name, tags=tags, description=self.mlflow_run_description) as run:
            ...
            print(f"Training complete - run id: {self.run_id}, experiment: {self.mlflow_experiment_location}")
