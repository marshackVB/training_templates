...
import mlflow
from training_templates.training_templates.sklearn_templates import SkLearnHyperoptBase
...
def test_join_to_pandas(spark, feature_table):
    ...
    trainer = TestSklearnHyperoptBase(feature_table, f"{feature_table}_train", **training_args)
...
@pytest.mark.parametrize("spark, feature_table, train_size, random_state, shuffle", [("spark", "feature_table", 0.7, 123, False),
                                                                                     ("spark", "feature_table", 0.6, 123, True),
                                                                                     ("spark", "feature_table", 0.5, 999, True)])
def test_proportions_split_train_val_test(spark, feature_table, train_size, random_state, shuffle, request):
    ...
    trainer = TestSklearnHyperoptBase(feature_table, f"{feature_table}_train", **updated_args)
