import pandas as pd
import pytest
from dev.data_utils import train_test_split
from dev.tests.sample_data_gen import sample_spark_dataframe



@pytest.mark.parametrize("train_size", [0.1, 0.5, 0.8])
def test_train_test_split_proportions(spark, train_size):
    ...
    train, test = train_test_split(feature_table_name=table_name,
    ...
    assert actual_train_size == train_size
...
def test_train_test_split_allow_overwrite(spark):
    ...
    train, test = train_test_split(feature_table_name=table_name,
                                    unique_id='PassengerId', 
                                    train_size=starting_train_size,
                                    allow_overwrite=True)
